/*
 * SettingFlog.cpp : ソーシャルブックマーク管理プログラム
 *      設定ダイアログ(Flog) インプリメンテーション ファイル
 *          written by H.Tsujimura  28 Jul 2006
 *
 * $Log: /comm/bookey/SettingFlog.cpp $
 * 
...
#ifndef	lint
static char	*rcs_id =
"$Header: /comm/bookey/SettingFlog.cpp 1     09/05/14 3:48 tsupo $";
#endif
...
SettingFlog::SettingFlog(CWnd* pParent /*=NULL*/)
	: CTabDialog(SettingFlog::IDD, pParent)
{
	//{{AFX_DATA_INIT(SettingFlog)
	//}}AFX_DATA_INIT
}

void SettingFlog::DoDataExchange(CDataExchange* pDX)
{
	CTabDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(SettingFlog)
	//}}AFX_DATA_MAP
}

// 「設定ダイアログ」での「適用」ボタン押下時処理
void SettingFlog::Accept()
{
    if ( m_initialized ) {
        CBookeyApp  *pp = (CBookeyApp *)AfxGetApp();

        pp->m_flog.m_username = m_username;
        pp->m_flog.m_password = m_password;
        pp->m_flog = m_isTarget;
    }
}

void SettingFlog::LoadSetting()
{
    if ( !m_initialized ) {
        CBookeyApp  *pp = (CBookeyApp *)AfxGetApp();

        m_username = pp->m_flog.m_username;
        m_password = pp->m_flog.m_password;
        m_isTarget = (bool)(pp->m_flog);
    }
}

BEGIN_MESSAGE_MAP(SettingFlog, CTabDialog)
	//{{AFX_MSG_MAP(SettingFlog)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
