/*
 * SettingMMmemo.cpp : ソーシャルブックマーク管理プログラム
 *      設定ダイアログ(MM/Memo) インプリメンテーション ファイル
 *          written by H.Tsujimura  28 Jul 2006
 *
 * $Log: /comm/bookey/SettingMMmemo.cpp $
 * 
...
#ifndef	lint
static char	*rcs_id =
"$Header: /comm/bookey/SettingMMmemo.cpp 1     09/05/14 3:48 tsupo $";
#endif
...
SettingMMmemo::SettingMMmemo(CWnd* pParent /*=NULL*/)
	: CTabDialog(SettingMMmemo::IDD, pParent)
{
	//{{AFX_DATA_INIT(SettingMMmemo)
	//}}AFX_DATA_INIT
}

void SettingMMmemo::DoDataExchange(CDataExchange* pDX)
{
	CTabDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(SettingMMmemo)
	//}}AFX_DATA_MAP
}

// 「設定ダイアログ」での「適用」ボタン押下時処理
void SettingMMmemo::Accept()
{
    if ( m_initialized ) {
        CBookeyApp  *pp = (CBookeyApp *)AfxGetApp();

        pp->m_mmMemo.m_username = m_username;
        pp->m_mmMemo.m_password = m_password;
        pp->m_mmMemo = m_isTarget;
    }
}

void SettingMMmemo::LoadSetting()
{
    if ( !m_initialized ) {
        CBookeyApp  *pp = (CBookeyApp *)AfxGetApp();

        m_username = pp->m_mmMemo.m_username;
        m_password = pp->m_mmMemo.m_password;
        m_isTarget = (bool)(pp->m_mmMemo);
    }
}

BEGIN_MESSAGE_MAP(SettingMMmemo, CTabDialog)
	//{{AFX_MSG_MAP(SettingMMmemo)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
